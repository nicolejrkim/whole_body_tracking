...
submit_job() {
    ...
    ssh $CLUSTER_LOGIN "cd $WORKSPACE_DIR && LOCAL_HOSTNAME=$(hostname -s) bash $WORKSPACE_DIR/$CLUSTER_WBT_DIRNAME/docker/cluster/$job_script_file \"${WORKSPACE_DIR}/$CLUSTER_WBT_DIRNAME\" \"$profile\" ${@}"
}
...
case $command in
    push)
        ...
        # rm -rf /$SCRIPT_DIR/exports/isaac-$profile*
    ...
    job)
        ...
        WORKSPACE_DIR="${CLUSTER_WORKSPACE_DIR}_${current_datetime}"
        ...
        # make sure target directory exists
        ssh $CLUSTER_LOGIN "mkdir -p $CLUSTER_WORKSPACE_DIR"
        # Sync Workspace code
        echo "[INFO] Syncing Workspace code..."
        rsync -rh --info=progress2  --exclude="*.git*" --filter=':- .dockerignore'  /$SCRIPT_DIR/../../.. $CLUSTER_LOGIN:$WORKSPACE_DIR
        # execute job script
        echo "[INFO] Executing job script..."
        # check whether the second argument is a profile or a job argument
        submit_output=$(submit_job $job_args)
        echo "$submit_output"
        # Parse job ID from sbatch output (e.g. "Submitted batch job 61040526")
        job_id=$(echo "$submit_output" | grep -oP 'Submitted batch job \K[0-9]+')
        # Log the submission locally
        local_log="$SCRIPT_DIR/submission_history.log"
        echo "$(date +'%Y-%m-%d %H:%M:%S') | host=$(hostname) | job=$job_id | profile=$profile | workspace=$WORKSPACE_DIR | args: $job_args" >> "$local_log"
        echo "[INFO] Job ID: $job_id"
        echo "[INFO] Submission logged to $local_log"
        ;;
    *)
