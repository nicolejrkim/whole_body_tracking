...
class ContainerInterface:
    ...
    def __init__(
        ...
        Args:
            ...
            name: The name for the container. Defaults to "whole-body-tracking".
            profile: The profile for the container. Defaults to "".
        ...
        if self.profile != "":
            self.container_name = f"{self.name}-{self.profile}"
            self.image_name = f"{self.name}-{self.profile}"
        else:
            self.container_name = f"{self.name}"
            self.image_name = f"{self.name}"

        # print the container and image names
        print(f"[INFO] Using container name: {self.container_name}")
        print(f"[INFO] Using image name: {self.image_name}")
    ...
    def build(self):
        ...
        print(f"[INFO] Building the docker image for {self.image_name}...\n")

        # build the image for the base profile
        subprocess.run(
            ["docker", "compose"] + self.add_yamls + self.add_profiles + self.add_env_files + ["build"],
            check=False,
            ...
            for container_path, host_path in artifacts.items():
                subprocess.run(
                    [
                        "docker",
                        "cp",
                        f"{self.image_name}:{container_path}/",
                        f"{host_path}",
                    ],
                    check=False,
                )
    ...
    def _resolve_image_extension(self, yamls: list[str] | None = None, envs: list[str] | None = None):
        ...
        # check if file .env.base exists
        if not (self.context_dir / ".env.base").exists():
            print(f"The file '.env.base' does not exist in the context directory {self.context_dir}. Ignoring it.")
            self.add_env_files = []
        else:
