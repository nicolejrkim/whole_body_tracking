...
def create_x11_tmpfile(tmpfile: Path | None = None, tmpdir: Path | None = None) -> Path:
    ...
    if display is None:
        print("[ERROR] The 'DISPLAY' environment variable is not set.")
        print("[ERROR] X11 forwarding requires a running display. Either:")
        print("[ERROR]   - Run this script in a graphical session, or")
        print("[ERROR]   - Disable X11 forwarding by setting 'X11_FORWARDING_ENABLED=0' in '.container.cfg'.")
        exit(1)
